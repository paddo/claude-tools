    ...
    - hooks:
        - type: command
          command: |
            ...
            [ -z "$TRANSCRIPT" ] && exit 0
            [ ! -f "$TRANSCRIPT" ] && exit 0
            grep -q 'kimi-code/bin/kimi' "$TRANSCRIPT" && exit 0
            echo "You are about to finish without having run Kimi K3. This agent exists to return a second opinion from a different model, and your own reasoning is not one: it comes from the same model that called you. Run the Kimi CLI as documented, then report its findings. If it errors or is missing, run it anyway so the attempt is on record, and say so in one line rather than analysing the code yourself." >&2
            exit 2
---
...
Running Kimi K3 is the job, not an aid to it. The whole worth of this
...
usable, say exactly that in one line and stop there. Name K3 in your
...
- Not a stand-in - if the CLI cannot run, report that; never substitute your own review for K3's
